...
Lastly, we define a set of key variables names used in the code below as well as the range of years, for which we estimate our poverty estimates. In some areas with a very small sample size, it may occur, that the individual data only consists of zeros and ones, resulting in a direct estimate of zero or one and a direct variance of zero. We set those areas to out-of-sample and for the final estimation results only the synthetic part of the FH model is used.
...
#   vardir = "vardir", combined_data = dta_ufh, domains = "id",
#   method = "ml", MSE = TRUE) 
```
