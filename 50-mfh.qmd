...
colnames(direct_dt)[colnames(direct_dt) == "subcode"] <- "ID"
...
widerhs_dt <- clean_rhs(rhs_dt = widerhs_dt)
...
area_vars <- c("ID", "No.", "Subregion") ### both variables are at the same level. if the levels vary, you would need to combine both variables for effect use
...
year_set <- unique(direct_dt$year)[2:4]
...
mfh_dt <- 
  ...
  pivot_wider(
    ...
    names_glue = "{.value}{year}",
    values_fn = ~ dplyr::first(na.omit(.x))  # or `first`, or something more explicit if neede
  ) |>
  merge(y = widerhs_dt,
        by = area_vars[[1]]) 
...
x_vars <- names(mfh_dt) %>% str_subset("^X\\d+y\\d+")

numeric_x_vars <- x_vars[sapply(mfh_dt[x_vars], is.numeric)]
...
candidate_vars <- c(numeric_x_vars,
                    paste0("log", numeric_x_vars),
                    paste0("scaled", numeric_x_vars))
...
fh_step <-
  lapply(X = paste0(directpov, unique(direct_dt[[year_var]])),
         FUN = function(x){
...
mfh_formula <-
  ...
  x = paste0(directpov, unique(direct_dt[[year_var]])),
  rhs = fh_step,
  SIMPLIFY = FALSE)
...
### create the appropriate set of variance covariance variables depending on what years of data are selected
...
varcols <- c(paste0(directvar, year_set),
             prepare_vcov())
