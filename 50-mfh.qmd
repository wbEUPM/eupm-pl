...
dta_full <-
  rhs_dta_log |> 
  left_join(pov_direct |> dplyr::filter(type == "arop") |> 
              dplyr::select(all_of(c(area_var, year_var, directpov, directvar)))) |> 
  dplyr::filter(!is.na(pov)) |> 
...
dta_full_wide <- 
  ...
  pivot_wider(
    ...
    names_sep = "_") |> 
  dplyr::select(id, contains(directpov), contains(directvar), everything())
...
form_pooled <- formula(model_pooled)
```

### Checking regression results for selected variables
...
First, we develop functions that allow estimating MFH models
...
eblup_zero_cov <-
  ...
  est_ufh_mfh(dta = _, formula = mfh_formula, verbose = T) |>
  mutate(model = str_c(model, " (0 cov)"))

bd_out |> pin_write(eblup_zero_cov, type = "rds")
```

Next, we estimate MVFH with variances constant for all regions.

```{r mvfh-constant-cov-est, eval=FALSE}
eblup_const_cov <-
  dta_mhf |> 
  mutate(
    across(contains("cov"), ~ . / 4),
    across(all_of(c(
  ...
  est_ufh_mfh(dta = _, formula = mfh_formula, verbose = T, precision = 0.001) |>
...
mod_dta_out <-
  ...
  mutate(model = ifelse(is.na(estimate), model, estimate)) |> 
  select(-estimate) |> 
  unnest(results) |> 
  mutate(estimate = model) 

bd_clean |> pin_write(mod_dta_out, name = "pov_mfh", type = "rds")
```
