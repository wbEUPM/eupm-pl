...
pak_list <- c("sf", "data.table", "tidyverse", "car", "msae",
              "sae", "survey", "spdep", "knitr", "MASS", "caret",
              "caret", "purrr", "pins", "gt", "lmtest", "scales",
              "viridis")

invisible(sapply(pak_list, 
                 library, 
                 character.only = TRUE))
...
#### now we estimate the model of the univariate models as well as the multivariate fay herriot models 1 and 2

model0_obj <- eblupUFH(mfh_formula, vardir = varcols, data = mfh_dt)
model1_obj <- eblupMFH1(mfh_formula, vardir = varcols, data = mfh_dt, MAXITER = 1e10, PRECISION = 1e-2)
model2_obj <- eblupMFH2(mfh_formula, vardir = varcols, data = mfh_dt, MAXITER = 1e10, PRECISION = 1e-2)
...
### Some Other Post-Estimation Analytics

Now that we have estimated the MFH models, we might be interested in the following:
