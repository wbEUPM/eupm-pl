...
dta_mhf_cov1 <- 
  dta_mhf |> 
  mutate(vov1 = cov2019_2020,
         # vov2 = cov2020_2021,
         # vov3 = cov2021_2022,
         # vov4 = cov2022_2023,
         across(contains("cov"), ~ 0),
         cov2019_2020 = vov1#,
         # cov2020_2021 = vov2,
         # cov2021_2023 = vov3,
         # cov2022_2023 = vov4
         ) |> 
  select(-contains("vcov"))

y_cols <- dta_mhf |> select(contains("pov")) |> names()
var_cols <- dta_mhf |> select(contains("var"), contains("cov")) |> names()

# Univariate FH
model0_obj <- eblupUFH(mfh_formula, vardir = var_cols, data = dta_mhf)
bd_out |> pin_write(model0_obj, type = "rds")

# multivariate FH 1
tic(msg = "eblupMFH1")
model1_obj <- eblupMFH1_v2(mfh_formula, 
                        vardir = var_cols, 
                        data = dta_mhf_cov1, 
                        MAXITER = 100, 
                        PRECISION = 0.01)
...
model0_obj$fit$estcoef |> all.equal(model1_obj$fit$estcoef)
model0_obj$eblup |> all.equal(model1_obj$eblup)
...
# multivariate FH 2
tic(msg = "eblupMFH2")
model2_obj <- eblupMFH2_v2(mfh_formula, 
                        vardir = var_cols, 
                        data = dta_mhf_cov1, 
                        MAXITER = 10000, 
                        PRECISION = 0.01)
...
pov_mfh <- 
  ext_mfh_dta(model0_obj,  select(dta_mhf, id), type = "arop", estimate = "UFH (MFH)") |>
  bind_rows(
    ext_mfh_dta(model1_obj,  select(dta_mhf, id), type = "arop", estimate = "MFH-1")
  ) |> 
  bind_rows(
    ext_mfh_dta(model2_obj,  select(dta_mhf, id), type = "arop", estimate = "MFH-2")
  )
