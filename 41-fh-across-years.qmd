...
run_save_fh <- function(prefix, candidate_vars) {
...
datasummary(
  year ~ ( 
    weighted.mean * Arguments(w = direct_N_pop ) + SD) * (estimate) * pov,
  data = pov_dta |> mutate(year = as.factor(year)) |>
    filter(str_detect(estimate, "Direct|9|6")),
  fmt = 4
)
```
...
```{r}
pov_maps |>
  filter(str_detect(estimate2, "9|6")) |>
  arrange(estimate2) |> 
...
```{r}
pov_maps |>
  filter(str_detect(estimate2, "Dir|9|6")) |>
  arrange(estimate2) |> 
  # filter(year2 %in% c(2022, 2023)) |>
  pull(plot_CV) |>
  reduce(~ .x + .y)  +
