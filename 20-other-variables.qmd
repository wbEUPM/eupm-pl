...
pop |> filter(year == 2023) |>  plot_var("ntl_mean_mask", geoms)
```
...
bd_raw |> pin_read("gsm")  |> pl_decode_id() |>
  filter(type_LAU %in% c("0", "1", "2", "3")) |>
  select(id, year, var, val) |>  plot_var("lte_n", geoms)
```
...
```{r}
# meta_vars |> 
#   mutate(id_indicator = str_remove_all(id_indicator, "P")) |> 
#   select(-Indicator) |> 
#   left_join(meta_groups, by = join_by(id_indicator)) |> 
#   filter(is.na(Catagory))
#   count(Catagory )
# meta_groups |> 
#   count(id_indicator, sort = T)
#   left_join()
#   group_by(Catagory)
